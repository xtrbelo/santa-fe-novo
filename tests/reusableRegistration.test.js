...
test('Consulente usa cadastro mínimo com nome, contato e CPF', () => {
  const payload = buildReusableRegistrationPayload({ id, tipoCadastro: 'consulente' }, { nome: ' Maria ', contato: '(96) 99999-1111', cpf: '529.982.247-25' });
...
test('Membro exige CPF válido e dados completos da Casa', () => {
  const payload = buildReusableRegistrationPayload({ id, tipoCadastro: 'membro' }, { nome: 'João', cpf: '529.982.247-25', contato: '96999991111', email: 'joao@example.test', dadosCasa: { dataIngresso: '', batizadoCaesf: false, dataBatismoCaesf: '' } });
...
test('nenhum dos formulários concede acesso ou cria Pessoa diretamente', () => {
  const payload = buildReusableRegistrationPayload({ id, tipoCadastro: 'consulente' }, { nome: 'Maria', contato: '96999991111', cpf: '52998224725' });
