...
export function AutocadastroMembroPage() {
  ...
  const [form, setForm] = useState({ dataNascimento: '', contato: '', email: '', sexo: 'nao_informado', estadoCivil: 'nao_informado', endereco: emptyAddress });
  ...
  const submit = async event => {
    ...
    try { await submitMemberSelfRegistration({ inviteId: invite.id, data: { ...form, nome: invite.nome, cpf: invite.cpf } }); setState({ status: 'respondido', invite }); }
      ...
      if (submitError.message === 'AUTOCADASTRO_EMAIL_INVALIDO') setError('Informe um e-mail válido.');
      else if (submitError.message === 'AUTOCADASTRO_CEP_INVALIDO') setError('Informe um CEP com 8 dígitos.');
      else if (submitError.message === 'AUTOCADASTRO_UF_INVALIDA') setError('Informe uma UF com 2 letras.');
      else if (submitError.message === 'AUTOCADASTRO_DATA_INVALIDA') setError('Informe uma data de nascimento válida.');
