...
import { createAgendamento, createConsulenteQuick, getAppCollection, getAppDoc, getDoc, onSnapshot, orderBy, query, updateAtendimentoServicos, where } from '../../services/firebase';
...
import { agendaAceitaServico, getNomeServicoAtendimento, getServicosAtivosAtendimento, servicoAtivoNaAgenda, servicoControlaVagas } from '../../utils/domain';
...
export const AgendasModule = ({ user, profile, returnRequest = null, onReturnConsumed }) => {
  ...
  const [editAppointment, setEditAppointment] = useState(null); const [editServices, setEditServices] = useState({}); const [savingEdit, setSavingEdit] = useState(false);
  useEffect(() => {
  ...
  const startEditServices = appointment => { setEditAppointment(appointment); setEditServices(Object.fromEntries(getServicosAtivosAtendimento(appointment).map(id => [id, true]))); };
  const compatibleEditServices = editAppointment ? services.filter(item => agendaAceitaServico(agendasById[editAppointment.agendaId], item.id) && servicoAtivoNaAgenda(agendasById[editAppointment.agendaId], item.id)) : [];
  const saveEditedServices = async () => {
    const selected = compatibleEditServices.filter(item => editServices[item.id]);
    if (!selected.length) { toast.error('O agendamento deve manter pelo menos um serviço.'); return; }
    setSavingEdit(true);
    try { await updateAtendimentoServicos({ agendaId: editAppointment.agendaId, agendamentoId: editAppointment.id, servicos: selected, userId: user.uid, responsavelNome: profile?.nome || user.displayName || user.email }); toast.success('Serviços do agendamento atualizados.'); setEditAppointment(null); }
    catch (error) { console.error(error); const code = error.message.split(':')[0]; const messages = { SEM_VAGA: 'Não há vaga disponível para um dos serviços selecionados.', SERVICOS_SEM_ALTERACAO: 'Nenhuma alteração foi feita nos serviços.', ATENDIMENTO_NAO_EDITAVEL: 'Este agendamento não pode mais ser alterado.' }; toast.error(messages[code] || 'Não foi possível alterar os serviços do agendamento.'); }
  ...
  return <div className="space-y-6 pb-10">
    ...
    {!loadingData && !loadError && <div className="space-y-3">{pagination.items.map(item => { const itemAgenda = agendasById[item.agendaId]; const editable = item.status === 'Agendado' && isAgendaFuture(itemAgenda); return <Card key={item.id} className="flex justify-between gap-4"><div><strong>{item.nome || item.pessoaNome || 'Pessoa não identificada'}</strong><p className="text-xs text-gray-500">{getServicosAtivosAtendimento(item).map(id => getNomeServicoAtendimento(item, id)).join(', ') || 'Serviço não informado'}</p>{item.observacao && <p className="mt-1 text-xs text-gray-600"><strong>Observação:</strong> {item.observacao}</p>}<p className="text-xs font-bold text-indigo-700 mt-1">{itemAgenda?.data?.toDate?.().toLocaleDateString('pt-BR') || 'Data histórica indisponível'} · {itemAgenda?.horario || '--:--'} · {itemAgenda?.tipoTrabalhoNome || itemAgenda?.tipo || 'Trabalho não informado'}</p>{editable && <Button variant="ghost" className="mt-2 h-9 px-3 text-xs" onClick={() => startEditServices(item)}><Pencil size={15}/> Alterar serviços</Button>}</div><span className="text-xs font-black">{item.status}</span></Card>; })}{!visible.length && <div className="text-center py-16 bg-white rounded-3xl"><CalendarDays className="mx-auto text-gray-300"/><p className="text-xs text-gray-400 mt-3">Nenhum agendamento neste filtro</p></div>}</div>}
    ...
    </div></Modal><Modal isOpen={!!editAppointment} onClose={() => !savingEdit && setEditAppointment(null)} title="Alterar serviços do agendamento"><div className="space-y-4"><div className="rounded-xl border border-indigo-200 bg-indigo-50 p-3"><p className="text-sm font-black text-indigo-950">{editAppointment?.nome || editAppointment?.pessoaNome}</p><p className="mt-1 text-xs text-indigo-700">O atendimento continuará único e as vagas serão atualizadas automaticamente.</p></div>{compatibleEditServices.map(item => <label key={item.id} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 ${editServices[item.id] ? 'border-indigo-400 bg-indigo-50' : 'border-gray-100'}`}><input type="checkbox" checked={editServices[item.id] || false} onChange={() => setEditServices(current => ({ ...current, [item.id]: !current[item.id] }))} className="h-4 w-4 rounded text-indigo-600"/><span className="text-sm font-bold text-gray-700">{item.nome}</span></label>)}<div className="grid grid-cols-2 gap-3"><Button variant="secondary" disabled={savingEdit} onClick={() => setEditAppointment(null)}>Cancelar</Button><Button disabled={savingEdit} onClick={saveEditedServices}>{savingEdit ? 'Salvando...' : 'Salvar serviços'}</Button></div></div></Modal><PessoaFormModal isOpen={quickRegisterOpen} onClose={() => setQuickRegisterOpen(false)} onSaved={quickRegistered} user={user} allowedVinculos={['consulente']} createOperation={createConsulenteQuick}/>
  </div>;
};
