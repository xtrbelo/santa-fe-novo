...
export const PessoaFormModal = ({ isOpen, onClose, onSaved, user, initialName = '', allowedVinculos = ['consulente', 'membro'] }) => {
  ...
  const [vinculo, setVinculo] = useState(allowedVinculos[0] || 'consulente');
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title="Cadastrar nova pessoa">
    <form onSubmit={save} className="space-y-4">
      {allowedVinculos.length > 1 && <label className="block text-xs font-black text-gray-500 uppercase">Tipo<select value={vinculo} onChange={event => setVinculo(event.target.value)} className="mt-1 w-full bg-gray-50 p-3 rounded-xl text-sm"><option value="consulente">Consulente</option><option value="membro">Membro</option></select></label>}
