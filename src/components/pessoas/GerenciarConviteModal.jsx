...
export function GerenciarConviteModal({ invite, userId, userNames = {}, onClose, toast }) {
  ...
  return <Modal isOpen onClose={onClose} title="Gerenciar convite" maxWidth="max-w-xl">{newResult ? <InviteCreatedContent result={newResult} onCopy={copy} /> : <div className="space-y-5">
    <dl className="grid grid-cols-1 gap-4 rounded-2xl border border-gray-100 p-4 sm:grid-cols-2"><div><dt className="text-xs font-black uppercase text-gray-400">Situação efetiva</dt><dd className="mt-1 font-black capitalize text-purple-700">{getMemberInviteEffectiveStatus(invite)}</dd></div><div><dt className="text-xs font-black uppercase text-gray-400">Criado em</dt><dd className="mt-1 font-bold">{timestampText(invite.criadoEm)}</dd></div>{userNames[invite.criadoPor] && <div><dt className="text-xs font-black uppercase text-gray-400">Criado por</dt><dd className="mt-1 font-bold">{userNames[invite.criadoPor]}</dd></div>}<div><dt className="text-xs font-black uppercase text-gray-400">Expira em</dt><dd className="mt-1 font-bold">{timestampText(invite.expiraEm)}</dd></div>{invite.revogadoEm && <div><dt className="text-xs font-black uppercase text-gray-400">Revogado em</dt><dd className="mt-1 font-bold">{timestampText(invite.revogadoEm)}</dd></div>}{userNames[invite.revogadoPor] && <div><dt className="text-xs font-black uppercase text-gray-400">Revogado por</dt><dd className="mt-1 font-bold">{userNames[invite.revogadoPor]}</dd></div>}</dl><p className="text-xs font-bold text-gray-500">O token anterior não é armazenado e não pode ser exibido.</p><div className="grid gap-2 sm:grid-cols-2">{invite.status === 'ativo' && <Button variant="danger" disabled={submitting} onClick={revoke}><ShieldX size={16} /> Revogar convite</Button>}<Button variant="purple" disabled={submitting} onClick={reissue}><RefreshCw size={16} /> Reenviar convite</Button></div>
  </div>}</Modal>;
}
