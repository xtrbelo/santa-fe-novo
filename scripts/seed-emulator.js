...
try {
  await environment.withSecurityRulesDisabled(async context => {
    ...
    await Promise.all([
      ...
      write('usuarios', 'atendimento-local-teste', { uid: 'atendimento-local-teste', nome: 'Recepção Local', email: 'atendimento@local.invalid', role: 'atendimento', ativo: true, criadoEm: now, atualizadoEm: now }),
      write('usuarios', 'pendente-local-teste', { uid: 'pendente-local-teste', nome: 'Pendente Local', email: 'pendente@local.invalid', role: 'pendente', ativo: true, criadoEm: now, atualizadoEm: now }),
      ...
      write('pessoas', 'membro-cambone-8a', withPessoaSearchIndex({ nome: 'Membro Cambone Teste', vinculo: 'membro', funcoesCasa: ['cambone'], tipoPessoa: 'Cambone', ativo: true })),
      write('pessoas', 'membro-sem-funcao-8a', withPessoaSearchIndex({ nome: 'Membro Sem Função', vinculo: 'membro', funcoesCasa: [], tipoPessoa: 'Membro', ativo: true })),
